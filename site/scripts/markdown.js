...
var imageRef = new RegExp((_d = ["![(.*)]((d+))"], _d.raw = ["!\\[(.*)\\]\\((\\d+)\\)"], String.raw(_d)));
var inlinelinkregex = new RegExp(reference.source + (_e = ["("], _e.raw = ["\\("], String.raw(_e)) +
    url.source + (_f = ["(?:s+\"([^\"]*)\")?)"], _f.raw = ["(?:\\s+\"([^\"]*)\")?\\)"], String.raw(_f)), 'g');
