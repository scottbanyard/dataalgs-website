...
*/
exports.imageRef = new RegExp((_d = ["![([^]]+)]((?:(d+)|"], _d.raw = ["!\\[([^\\]]+)\\]\\((?:(\\d+)|"], String.raw(_d)) + exports.url.source + (_e = ["))"], _e.raw = [")\\)"], String.raw(_e)), 'g');
// Inline link regex
var inlinelinkregex = new RegExp(exports.reference.source + (_f = ["("], _f.raw = ["\\("], String.raw(_f)) +
    exports.url.source + (_g = ["(?:s+\"([^\"]*)\")?)"], _g.raw = ["(?:\\s+\"([^\"]*)\")?\\)"], String.raw(_g)), 'g');
...
*/
var scriptTag = [new RegExp((_h = ["<script\b[^<]*(?:(?!</script>)<[^<]*)*</script>"], _h.raw = ["<script\\b[^<]*(?:(?!<\\/script>)<[^<]*)*<\\/script>"], String.raw(_h)), 'gi'), ''];
